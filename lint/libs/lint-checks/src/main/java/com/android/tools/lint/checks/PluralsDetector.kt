...
class PluralsDetector : ResourceXmlDetector() {
    ...
    override fun getApplicableElements() = listOf(SdkConstants.TAG_PLURALS)
    ...
    override fun visitElement(context: XmlContext, element: Element) {
        ...
        while (i < n) {
            val noe = children.item(i)
            if (noe.nodeType != Node.ELEMENT_NODE) {
                i++
                continue
            }
            val child = noe as Element
            if (SdkConstants.TAG_ITEM != child.tagName) {
                i++
                continue
            }
            val quantityString = child.getAttribute(SdkConstants.ATTR_QUANTITY)
            ...
            if (plurals.hasMultipleValuesForQuantity(language, quantity) &&
                ...
                val message =
                    """
            The quantity `'$quantity'` matches more than one specific number in this locale$append, but the message did not include a
            formatting argument (such as `%%d`). This is usually an internationalization error. See full issue explanation for more.
                    """.trimIndent()
    ...
    companion object {
        ...
        val MISSING = create(
            ...
            explanation = """
        Different languages have different rules for grammatical agreement with quantity. In English, for example, the quantity 1 is \
        a special case. We write "1 book", but for any other quantity we'd write "n books". This distinction between singular and plural \
        is very common, but other languages make finer distinctions.

        This lint check looks at each translation of a `<plural>` and makes sure that all the quantity strings considered by the given \
        language are provided by this translation.

        For example, an English translation must provide a string for `quantity="one"`. Similarly, a Czech translation must provide a \
        string for `quantity="few"`.
            """.trimIndent(),
        ...
        val EXTRA = create(
            ...
            explanation = """
        Android defines a number of different quantity strings, such as `zero`, `one`, `few` and `many`. However, many languages do not \
        distinguish grammatically between all these different quantities.

        This lint check looks at the quantity strings defined for each translation and flags any quantity strings that are unused (because \
        the language does not make that quantity distinction, and Android will therefore not look it up).

        For example, in Chinese, only the `other` quantity is used, so even if you provide translations for `zero` and `one`, these \
        strings will **not** be returned when `getQuantityString()` is called, even with `0` or `1`.
            """.trimIndent(),
            category = Category.MESSAGES,
            priority = 3,
        ...
        val IMPLIED_QUANTITY = create(
            ...
            explanation = """
        Plural strings should generally include a `%s` or `%d` formatting argument. In locales like English, the `one` quantity only \
        applies to a single value, 1, but that's not true everywhere. For example, in Slovene, the `one` quantity will apply to 1, 101, \
        201, 301, and so on. Similarly, there are locales where multiple values match the `zero` and `two` quantities.

        In these locales, it is usually an error to have a message which does not include a formatting argument (such as '%d'), since it \
        will not be clear from the grammar what quantity the quantity string is describing.
            """.trimIndent(),
            category = Category.MESSAGES,
            priority = 5,
            severity = Severity.ERROR,
