...
public class GradleDetector extends Detector implements GradleScanner {
    ...
    protected static boolean isInterestingStatement(
            @NonNull String statement, @Nullable String parent) {
    ...
    protected void checkOctal(
            @NonNull Context context, @NonNull String value, @NonNull Object cookie) {
        if (value.length() >= 2
                && value.charAt(0) == '0'
                && (value.length() > 2 || value.charAt(1) >= '8' && isNonnegativeInteger(value))
                && context.isEnabled(ACCIDENTAL_OCTAL)) {
            String message =
                    "The leading 0 turns this number into octal which is probably "
    ...
    protected void checkDslPropertyAssignment(
        ...
        if (parent.equals("defaultConfig")) {
            ...
            if (property.equals("versionName")
                    || property.equals("versionCode") && !isNonnegativeInteger(value)
            ...
            if (versionString != null) {
                ...
                if (version != null) {
                    GradleVersion recommended =
                            getLatestBuildTools(context.getClient(), version.getMajor());
                    if (recommended != null && version.compareTo(recommended) < 0) {
                        String message =
                                "Old buildToolsVersion "
                                        ...
                                        + " or later";
                        LintFix fix =
                                getUpdateDependencyFix(version.toString(), recommended.toString());
            ...
            if (value.startsWith("files('") && value.endsWith("')")) {
                ...
                if (path.contains("\\\\")) {
                    String message =
                            "Do not use Windows file separators in .gradle files; "
                                    + "use / instead";
                ...
                if (dependency != null) {
                    ...
                    if (gc != null && dependency.contains("$")) {
                        if (value.startsWith("'")
                            ...
                            LintFix fix =
                                    fix().name("Replace single quotes with double quotes")
                                            .replace()
                                            .text(value)
                                            .with(
                                                    "\""
    ...
    @Nullable
    private static GradleCoordinate resolveCoordinate(
            @NonNull Context context, @NonNull GradleCoordinate gc) {
                ...
                if (mc != null
                    ...
                    if (!revisions.isEmpty()) {
                        return new GradleCoordinate(
                                mc.getGroupId(), mc.getArtifactId(), revisions, null);
                    }
                    break;
                }
            }
    ...
    private void checkIntegerAsString(Context context, String value, Object valueCookie) {
        ...
        if (isNumberString(string)) {
            String message =
                    String.format(
                            "Use an integer rather than a string here "
                                    + "(replace %1$s with just %2$s)",
                            value, string);
            LintFix fix =
                    fix().name("Replace with integer").replace().text(value).with(string).build();
    ...
    protected void checkMethodCall(
            ...
            @SuppressWarnings("UnusedParameters") @NonNull List<String> unnamedArguments,
        ...
        if (compileSdkVersion > 0 && targetSdkVersion > 0 && targetSdkVersion > compileSdkVersion) {
            String message =
                    "The compileSdkVersion ("
                            ...
                            + ")";
            LintFix fix =
                    fix().name("Set compileSdkVersion to " + targetSdkVersion)
                            .replace()
    ...
    private void checkDependency(
            ...
            case "com.android.support.test":
                {
                    ...
                    if (repository != null) {
                        GradleVersion max =
                                MavenRepositories.getHighestInstalledVersionNumber(
                                        groupId,
                                        artifactId,
                                        repository,
                                        filter,
        ...
        if (context.getScope().size() > 1 && context.isEnabled(REMOTE_VERSION)) {
            GradleVersion latest =
                    getLatestVersionFromRemoteRepo(
                            context.getClient(), dependency, filter, dependency.isPreview());
        ...
        newerVersion =
                GradleVersion.max(
                        newerVersion,
                        getHighestKnownVersion(context.getClient(), dependency, filter));
     ...
     */
    @Nullable
    private Predicate<GradleVersion> getUpgradeVersionFilter(
    ...
    @VisibleForTesting static GoogleMavenRepository googleMavenRepository;
    ...
    @Nullable
    private static GradleVersion getGoogleMavenRepoVersion(
            @NonNull Context context,
            @NonNull GradleCoordinate dependency,
        ...
        synchronized (GradleDetector.class) {
            if (googleMavenRepository == null) {
                ...
                googleMavenRepository =
                        new GoogleMavenRepository(cacheDir) {
                            @Override
                            @Nullable
                            ...
                            @Override
                            public void error(
                                    @NonNull Throwable throwable, @Nullable String message) {
    ...
    @Nullable
    private GradleVersion findCachedNewerVersion(
            GradleCoordinate dependency, @Nullable Predicate<GradleVersion> filter) {
        File versionDir =
                new File(
                        getArtifactCacheHome(),
                        dependency.getGroupId() + File.separator + dependency.getArtifactId());
        if (versionDir.exists()) {
            return MavenRepositories.getHighestVersion(
                    versionDir,
                    filter,
    ...
    private void ensureTargetCompatibleWithO(
            @NonNull Context context,
        ...
        if (version != null && !version.isAtLeast(major, minor, micro)) {
            ...
            String message =
                    String.format("Version must be at least %1$s when targeting O", revision);
    ...
    private static String getNewerVersionAvailableMessage(
            GradleCoordinate dependency, String version) {
    ...
    @Nullable
    public static GradleVersion getLatestVersionFromRemoteRepo(
            @NonNull LintClient client,
            @NonNull GradleCoordinate dependency,
            @Nullable Predicate<GradleVersion> filter,
    ...
    private boolean checkGradlePluginDependency(
            Context context, GradleCoordinate dependency, Object cookie) {
        GradleCoordinate minimum =
        ...
        if (isSupportLibraryDependentOnCompileSdk(groupId, artifactId)) {
            if (compileSdkVersion >= 18
                ...
                if (newerVersion != null) {
                    fix =
                            fix().name("Replace with " + newerVersion)
                                    .replace()
    ...
    private void checkPlayServices(
        ...
        if (GMS_GROUP_ID.equals(groupId) && "play-services-appindexing".equals(artifactId)) {
            ...
            LintFix fix =
                    fix().name("Replace with Firebase")
                            .replace()
    ...
    private void checkConsistentSupportLibraries(
            @NonNull Context context, @Nullable Object cookie) {
        checkConsistentLibraries(context, cookie, SUPPORT_LIB_GROUP_ID, null);
    }

    private void checkConsistentPlayServices(@NonNull Context context, @Nullable Object cookie) {
        checkConsistentLibraries(context, cookie, GMS_GROUP_ID, FIREBASE_GROUP_ID);
    }

    private void checkConsistentWearableLibraries(
            @NonNull Context context, @Nullable Object cookie) {
        ...
        for (JavaLibrary library : getJavaLibraries(project)) {
            ...
            if (coordinates != null
                ...
                if (!library.isProvided()) {
                    if (cookie != null) {
                        String message =
                                "This dependency should be marked as "
                        ...
                        String message =
                                String.format(
                                        "The %1$s:%2$s dependency should be "
                                                + "marked as `provided`, not `compile`",
                                        ANDROID_WEAR_GROUP_ID, WEARABLE_ARTIFACT_ID);
                        reportFatalCompatibilityIssue(
                                context, guessGradleLocation(context.getProject()), message);
                    }
                }
        ...
        if (!supportVersions.isEmpty()) {
            if (wearableVersions.isEmpty()) {
                ...
                String message =
                        String.format(
                                "Project depends on %1$s:%2$s:%3$s, so it must "
                ...
                if (!supportVersions.equals(wearableVersions)) {
                    ...
                    String message =
                            String.format(
                                    ...
                                    GOOGLE_SUPPORT_GROUP_ID,
                                    ANDROID_WEAR_GROUP_ID,
                                    sortedSupportVersions.size() == 1
                                            ? sortedSupportVersions.get(0)
                                            : sortedSupportVersions.toString(),
                                    supportedWearableVersions.size() == 1
                                            ? supportedWearableVersions.get(0)
    ...
    private void checkConsistentLibraries(
            @NonNull Context context,
        ...
        for (AndroidLibrary library : androidLibraries) {
            ...
            //noinspection ConstantConditions
            if (coordinates != null
                    && (coordinates.getGroupId().equals(groupId)
        ...
        if (!dependencies.isEmpty()) {
            for (List<Library> path : dependencies) {
                ...
                if (location.getStart() == null) {
                    location =
                            guessGradleLocation(
                                    context.getClient(), projectDir, coordinates.getArtifactId());
                }
    ...
    @Nullable
    private static GradleVersion getNewerVersion(
            @NonNull GradleVersion version1, int major, int minor, int micro) {
    ...
    @Nullable
    private static GradleVersion getNewerVersion(
            @NonNull GradleVersion version1,
    ...
    @Nullable
    private static GradleVersion getNewerVersion(
            @NonNull GradleVersion version1, @NonNull GradleVersion version2) {
     ...
     *
     * <p>However, for {@link #COMPATIBILITY} we want to treat some issues as fatal (breaking the
     ...
     * {@link #reportNonFatalCompatibilityIssue(Context, Object, String)} which can be used to
    ...
    private void reportFatalCompatibilityIssue(
            @NonNull Context context, @NonNull Object cookie, @NonNull String message) {
    ...
    private static void reportFatalCompatibilityIssue(
            @NonNull Context context, @NonNull Location location, @NonNull String message) {
