...
abstract class GooglePlaySdkIndex(val client: LintClient, cacheDir: Path? = null) : NetworkCache(
    ...
    protected open fun logHasCriticalIssues(groupId: String, artifactId: String, versionString: String, file: File?) {
        if (showCriticalIssues) {
            client.log(Severity.INFORMATIONAL, null,"$groupId:$artifactId:$versionString has messages from its author")
        }
    }

    protected open fun logNonCompliant(groupId: String, artifactId: String, versionString: String, file: File?) {
        if (showPolicyIssues) {
            client.log(Severity.WARNING, null,"$groupId:$artifactId:$versionString has policy issues that will block publishing")
        }
    }

    protected open fun logOutdated(groupId: String, artifactId: String, versionString: String, file: File?) {
        client.log(Severity.INFORMATIONAL, null, "$groupId:$artifactId:$versionString has been marked as outdated by its author")
    }

    protected open fun logCachingError(message: String?) {
        client.log(Severity.WARNING, null,"Could not read index from network cache: $message")
    }

    protected open fun logErrorInDefaultData(message: String?) {
        client.log(Severity.WARNING, null,"Could not read index from default data: $message")
    }

    protected open fun logIndexLoadedCorrectly() {
        client.log(Severity.INFORMATIONAL, null,"Index data read correctly")
    }
}
