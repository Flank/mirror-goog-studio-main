...
abstract class GooglePlaySdkIndex(cacheDir: Path? = null) : NetworkCache(
    ...
    fun initialize(overriddenData: InputStream? = null) {
        ...
        if (overriddenData != null) {
            ...
            try {
                ...
                try {
                    index = Index.parseFrom(GZIPInputStream(readDefaultData("")))
    ...
    override fun readDefaultData(relative: String): InputStream? {
        return GooglePlaySdkIndex::class.java.getResourceAsStream("/$GOOGLE_PLAY_SDK_INDEX_SNAPSHOT_FILE")
    }
    ...
    protected open fun logHasCriticalIssues(groupId: String, artifactId: String, versionString: String, file: File?) {
        if (showCriticalIssues) {
            Logger.getInstance(this::class.java).info("$groupId:$artifactId:$versionString has messages from its author")
        }
    }

    protected open fun logNonCompliant(groupId: String, artifactId: String, versionString: String, file: File?) {
        if (showPolicyIssues) {
            Logger.getInstance(this::class.java).warn("$groupId:$artifactId:$versionString has policy issues that will block publishing")
        }
    }

    protected open fun logOutdated(groupId: String, artifactId: String, versionString: String, file: File?) {
        Logger.getInstance(this::class.java).info("$groupId:$artifactId:$versionString has been marked as outdated by its author")
    }

    protected open fun logCachingError(message: String?) {
        Logger.getInstance(this::class.java).warn("Could not read index from network cache: $message")
    }

    protected open fun logErrorInDefaultData(message: String?) {
        Logger.getInstance(this::class.java).warn("Could not read index from default data: $message")
    }

    protected open fun logIndexLoadedCorrectly() {
        Logger.getInstance(this::class.java).info("Index data read correctly")
    }
}
