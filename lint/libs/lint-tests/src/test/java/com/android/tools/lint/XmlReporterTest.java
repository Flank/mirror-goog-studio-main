...
public class XmlReporterTest extends AbstractCheckTest {
        ...
        try {
            ...
            assertEquals(
                    ""
                            ...
                            + "        <location\n"
                            + "            file=\""
                            + new File("/foo/Foo/AndroidManifest.xml").getAbsolutePath()
                            + "\"\n"
                            + "            line=\"7\"\n"
                            + "            column=\"5\"/>\n"
                            + "    </issue>\n"
                            ...
                            + "        <location\n"
                            + "            file=\""
                            + new File("/foo/bar/Foo/res/layout/main.xml").getAbsolutePath()
                            + "\"\n"
                            + "            line=\"12\"\n"
                            + "            column=\"9\"/>\n"
                            + "    </issue>\n"
    ...
    public void testNonPrintableChars() throws Exception {
        ...
        try {
            ...
            warning1.location =
                    Location.create(warning1.file, new DefaultPosition(592, 46, -1), null);
            ...
            String explanation =
                    ((Element) document.getElementsByTagName("issue").item(0))
                            .getAttribute("explanation");
            assertEquals(TypographyDetector.FRACTIONS.getExplanation(TextFormat.RAW), explanation);
        } finally {
            //noinspection ResultOfMethodCallIgnored
            file.delete();
        }
