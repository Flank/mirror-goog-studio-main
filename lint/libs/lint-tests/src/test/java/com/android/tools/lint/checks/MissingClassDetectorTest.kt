...
class MissingClassDetectorTest : AbstractCheckTest() {
    ...
    fun testIncrementalInManifest() {
        lint()
            .issues(MISSING, INSTANTIATABLE, INNERCLASS)
            .customScope(Scope.MANIFEST_SCOPE)
            .files(androidManifestWrongRegs)
            .run()
            .expectClean()
    }

    fun test() {
        lint().issues(MISSING).files(androidManifestWrongRegs, apiCallTest).run().expect(
            """
    ...
    fun testLibraryProjects() {
        val library = ProjectDescription(
        ...
        val master = ProjectDescription(
            mAndroidManifestRegs,
            mTestService,
            mTestService_class,
            classpath()
        ).name("MasterProject").dependsOn(library)

        lint()
            .issues(MISSING, INSTANTIATABLE, INNERCLASS)
            .projects(master, library)
            .run()
            .expect(
                """
    ...
    fun testIndirectLibraryProjects() {
        val library = ProjectDescription(
            manifest().pkg("foo.library").minSdk(14),
            projectProperties().library(true).compileSdk(14),
            mOnClickActivity,
            ...
            mTestProvider_class,
            mTestProvider2,
            mTestProvider2_class // Missing TestReceiver: Test should complain about just that class
        ).type(LIBRARY).name("RealLibrary")

        val library2 = ProjectDescription(
            xml(
                "AndroidManifest.xml",
                "" +
            ...
            projectProperties()
                .library(true)
                .compileSdk(14)
        ).type(LIBRARY).name("LibraryProject").dependsOn(library)
        ...
        val master = ProjectDescription(
            ...
            mTestService_class,
            classpath()
        ).name("MasterProject").dependsOn(library2)

        lint()
            .issues(MISSING, INSTANTIATABLE, INNERCLASS)
            .projects(master, library, library2)
            .run()
            .expect(
                """
                MasterProject/AndroidManifest.xml:32: Error: Class referenced in the manifest, test.pkg.TestReceiver, was not found in the project or the libraries [MissingRegistered]
    ...
    fun testLibraryWithMissingClass() {
        lint()
            .issues(MISSING)
            .files(
                xml(
    ...
    fun testLibraryWithMissingClassInApp() {
        val library = project(
            xml(
                "AndroidManifest.xml",
                "" +
    ...
    fun testInnerClassStatic() {
        lint()
            ...
            .files(
                ...
                mFoo_class,
                base64gzip(
                    "bin/classes/test/pkg/Foo\$Bar.class",
                    "" +
                ...
                base64gzip(
                    "bin/classes/test/pkg/Foo\$Baz.class",
                    "" +
    ...
    fun testInnerClassPublic() {
        lint()
            .issues(MISSING, INSTANTIATABLE, INNERCLASS)
            .files(mAndroidManifestInner, classpath(), mBar, mBar_class)
            .run()
            .expect(
                """
    ...
    fun testInnerClass() {
        lint()
            .files(mAndroidManifest, classpath(), apiCallTest, mFoo)
            .issues(
                MISSING,
                INSTANTIATABLE,
                INNERCLASS
    ...
    fun testInnerClass2() {
        lint()
            .issues(MISSING, INSTANTIATABLE, INNERCLASS)
            .files(mAndroidManifestInner, classpath(), apiCallTest, mBar)
            .run()
            .expect(
                """
    ...
    fun testNoClassesWithLibraries() {
        lint()
            .issues(MISSING, INSTANTIATABLE, INNERCLASS)
            .files(
                androidManifestWrongRegs,
                classpath(),
                base64gzip(
                    "libs/foo.jar",
                    "" +
    ...
    fun testCustomViewInCapitalizedPackage() {
        lint()
            .issues(MISSING, INSTANTIATABLE, INNERCLASS)
            .files(
                xml(
    ...
    fun testCustomViewNotReferenced() {
        lint()
            .issues(MISSING, INSTANTIATABLE, INNERCLASS)
            .files(mCustomView3, mCustomView3_class)
            .run()
            .expectClean()
    }
    ...
    fun testMissingClass() {
        lint()
            ...
            .files(
                ...
                base64gzip(
                    "bin/classes/course/examples/DataManagement/PreferenceActivity/ViewAndUpdatePreferencesActivity\$UserPreferenceFragment.class",
                    "" +
    ...
    private val mAndroidManifest = xml(
        "AndroidManifest.xml",
        "" +
                "<manifest xmlns:android=\"http://schemas.android.com/apk/res/android\"\n" +
                "    package=\"test.pkg\"\n" +
    ...
    private val mAndroidManifestInner = xml(
        "AndroidManifest.xml",
        "" +
                "<manifest xmlns:android=\"http://schemas.android.com/apk/res/android\"\n" +
                "    package=\"test.pkg.Foo\"\n" +
    ...
    private val mAndroidManifestRegs = xml(
        "AndroidManifest.xml",
        "" +
                "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
                "<!--\n" +
    ...
    private val androidManifestWrongRegs = xml(
        "AndroidManifest.xml",
        "" +
                "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
                "<manifest xmlns:android=\"http://schemas.android.com/apk/res/android\"\n" +
    ...
    private val apiCallTest: TestFile = base64gzip(
    ...
    private val mBar_class: TestFile = base64gzip(
        "bin/classes/test/pkg/Foo/Bar.class",
        "" +
    ...
    private val mCustomView3_class: TestFile = base64gzip(
        "bin/classes/test/bytecode/CustomView3.class",
        "" +
    ...
    private val mFoo_class: TestFile = base64gzip(
        "bin/classes/test/pkg/Foo.class",
        "" +
    ...
    private val mFragmentTest = java(
        "" +
                "package test.pkg;\n" +
                "\n" +
    ...
    private val `mFragmentTest$Fragment1`: TestFile = base64gzip(
        "bin/classes/test/pkg/FragmentTest\$Fragment1.class",
        "" +
    ...
    private val `mFragmentTest$Fragment2`: TestFile = base64gzip(
        "bin/classes/test/pkg/FragmentTest\$Fragment2.class",
        "" +
    ...
    private val `mFragmentTest$Fragment3`: TestFile = base64gzip(
        "bin/classes/test/pkg/FragmentTest\$Fragment3.class",
        "" +
    ...
    private val `mFragmentTest$Fragment4`: TestFile = base64gzip(
        "bin/classes/test/pkg/FragmentTest\$Fragment4.class",
        "" +
    ...
    private val `mFragmentTest$Fragment5`: TestFile = base64gzip(
        "bin/classes/test/pkg/FragmentTest\$Fragment5.class",
        "" +
    ...
    private val `mFragmentTest$Fragment6`: TestFile = base64gzip(
        "bin/classes/test/pkg/FragmentTest\$Fragment6.class",
        "" +
    ...
    private val `mFragmentTest$NotAFragment`: TestFile =
        base64gzip(
            "bin/classes/test/pkg/FragmentTest\$NotAFragment.class",
            "" +
    ...
    private val mOnClickActivity = java(
        "" +
                "package test.pkg;\n" +
                "\n" +
                "import android.app.Activity;\n" +
    ...
    private val mOnClickActivity_class: TestFile = base64gzip(
        "bin/classes/test/pkg/OnClickActivity.class",
        "" +
    ...
    private val mTestProvider = java(
        "" +
                "package test.pkg;\n" +
                "\n" +
    ...
    private val mTestProvider_class: TestFile = base64gzip(
        "bin/classes/test/pkg/TestProvider.class",
        "" +
    ...
    private val mTestProvider2 = java(
        "" +
                "package test.pkg;\n" +
                "\n" +
    ...
    private val mTestProvider2_class: TestFile = base64gzip(
    ...
    private val mTestReceiver = java(
        "" +
                "package test.pkg;\n" +
                "\n" +
    ...
    private val mTestReceiver_class: TestFile = base64gzip(
        "bin/classes/test/pkg/TestReceiver.class",
        "" +
    ...
    private val mTestService_class: TestFile = base64gzip(
        "bin/classes/test/pkg/TestService.class",
        "" +
