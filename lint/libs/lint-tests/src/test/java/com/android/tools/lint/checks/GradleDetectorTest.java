...
public class GradleDetectorTest extends AbstractCheckTest {
        ...
        lint().files(
                ...
                .modifyGradleMocks(
                        (project, variant) -> {
                            ...
                            JavaLibrary library2 =
                                    dependencies.getJavaLibraries().iterator().next();
        ...
        lint().files(
                        gradle(
                                ""
                                        ...
                                        + "}\n"))
                .issues(COMPATIBILITY)
                .client(getClientWithMockPlatformTarget(new AndroidVersion("25"), 1))
                .sdkHome(getMockSupportLibraryInstallation())
                .run()
                .expectClean();
        ...
        lint().files(
                        gradle(
                                ""
                                        ...
                                        + "}\n"))
                .issues(COMPATIBILITY)
                .client(getClientWithMockPlatformTarget(new AndroidVersion("26"), 2))
                .sdkHome(getMockSupportLibraryInstallation())
                .run()
                .expectClean();
        ...
        lint().files(
                        gradle(
                                ""
                                        ...
                                        + "}\n"))
                .issues(COMPATIBILITY)
                .client(getClientWithMockPlatformTarget(new AndroidVersion("O"), 2))
                .sdkHome(getMockSupportLibraryInstallation())
                .run()
                .expect(
                        ""
        ...
        lint().files(
                        gradle(
                                ""
                                        ...
                                        + "}\n"))
                .issues(COMPATIBILITY)
                .client(getClientWithMockPlatformTarget(new AndroidVersion("O"), 1))
                .sdkHome(getMockSupportLibraryInstallation())
                .run()
                .expect(
                        ""
        ...
        lint().files(
                        ...
                        kts(
                                ""
                                        ...
                                        + "}"))
                .sdkHome(getMockSupportLibraryInstallation())
                .issues(DEPENDENCY, MIN_SDK_TOO_LOW)
                .run()
                .expect(
                        "build.gradle.kts:7: Warning: Old buildToolsVersion 25.0.0; recommended version is 25.0.2 or later [GradleDependency]\n"
                                + "    buildToolsVersion(\"25.0.0\")\n"
                                + "                       ~~~~~~\n"
        ...
        private void visitQuietly(@NonNull final Context context) {
            ...
            GroovyCodeVisitor visitor =
                    new CodeVisitorSupport() {
                        private final List<MethodCallExpression> mMethodCallStack =
                                new ArrayList<>();
            ...
            if (node.getLastLineNumber() == -1 && node instanceof TupleExpression) {
                ...
                if (!expressions.isEmpty()) {
                    return Pair.of(
                            ...
                            getOffsets(expressions.get(expressions.size() - 1), context)
                                    .getSecond());
                }
            }
    ...
    private TestFile mDependencies =
            source(
                    "build.gradle",
                    ""
                            + "apply plugin: 'android'\n"
