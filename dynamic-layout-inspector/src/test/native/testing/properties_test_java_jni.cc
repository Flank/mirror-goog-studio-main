...
Java_com_android_tools_agent_layoutinspector_PropertiesTest_allocateEvent(
    JNIEnv *env, jclass clazz) {
  PropertyEvent *request = new PropertyEvent();
  return (long)request;
}

JNIEXPORT jbyteArray JNICALL
Java_com_android_tools_agent_layoutinspector_PropertiesTest_toByteArray(
    JNIEnv *env, jclass clazz, jlong event) {
  PropertyEvent *request = (PropertyEvent *)event;
  std::string str;
