...
public class CreateSymbolicLinkAction extends Action {
    ...
    public void execute(ILogger logger) {
        try {
            logger.verbose("Creating symbolic link \"%s\" pointing to target \"%s\"",
                    mDestination.getPath(), mDestination.getTarget());

            // Note: Does not throw if directory already exists
            Files.createDirectories(mDestination.getPath().getParent());
            Files.createSymbolicLink(mDestination.getPath(), mDestination.getTarget());
        }
        catch (IOException e) {
