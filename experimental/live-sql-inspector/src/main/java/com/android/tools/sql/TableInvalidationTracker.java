...
/**
 * TableInvalidationTracker keeps a list of tables modified by queries and notifies its callbacks
 * about these tables.
 */
// Some details on how the TableInvalidationTracker works:
...
// * Before each beginTransaction, RoomDatabase invokes TableInvalidationTracker to sync trigger states.
// * After each endTransaction, RoomDatabase invokes TableInvalidationTracker to refresh invalidated
// tables.
...
public final class TableInvalidationTracker {
    private static final String LOG_TAG = TableInvalidationTracker.class.getSimpleName();
    private static final String[] TRIGGERS = new String[] {"UPDATE", "DELETE", "INSERT"};

    private static final String UPDATE_TABLE_NAME = "inspector_table_modification_log";
    ...
    static final String RESET_UPDATED_TABLES_SQL =
            "UPDATE "
                    + UPDATE_TABLE_NAME
                    + " SET "
                    + INVALIDATED_COLUMN_NAME
    ...
    static final String SELECT_UPDATED_TABLES_SQL =
    ...
    @SuppressWarnings("WeakerAccess") /* synthetic access */
    volatile SQLiteStatement mCleanupStatement;

    private ObservedTableTracker mObservedTableTracker;

    // should be accessed with synchronization only.
    @SuppressLint("RestrictedApi")
    final Map<Observer, ObserverWrapper> mObserverMap = new HashMap<Observer, ObserverWrapper>();

    public static TableInvalidationTracker createInvalidationTracker(SQLiteDatabase database) {
        Cursor cursor =
                database.rawQuery(
                        "SELECT name FROM sqlite_master WHERE type='table'", new String[] {});
        ...
        TableInvalidationTracker invalidationTracker =
                new TableInvalidationTracker(database, tableNames.toArray(new String[0]));
        invalidationTracker.internalInit(database);
        return invalidationTracker;
    }

    private TableInvalidationTracker(SQLiteDatabase database, String... tableNames) {
        this(
                database,
    ...
    private TableInvalidationTracker(
            SQLiteDatabase database,
            Map<String, String> shadowTablesMap,
    ...
    private void internalInit(SQLiteDatabase database) {
        synchronized (this) {
            if (mInitialized) {
    ...
    private static void appendTriggerName(
            StringBuilder builder, String tableName, String triggerType) {
        builder.append("`")
                .append("inspector_table_modification_trigger_")
                .append(tableName)
                .append("_")
                .append(triggerType)
                .append("`");
    ...
    private void stopTrackingTable(SQLiteDatabase writableDb, int tableId) {
    ...
    private void startTrackingTable(SQLiteDatabase writableDb, int tableId) {
        writableDb.execSQL(
        ...
        for (String trigger : TRIGGERS) {
            ...
            stringBuilder
                    ...
                    .append(" SET ")
                    .append(INVALIDATED_COLUMN_NAME)
                    .append(" = 1")
                    .append(" WHERE ")
                    .append(TABLE_ID_COLUMN_NAME)
                    .append(" = ")
                    .append(tableId)
                    .append(" AND ")
                    .append(INVALIDATED_COLUMN_NAME)
     ...
     * observes changes.
     *
     * <p>Database changes are pulled on another thread so in some race conditions, the observer
     * might be invoked for changes that were done before it is added.
     *
     * <p>If the observer already exists, this is a no-op call.
     *
     * <p>If one of the tables in the Observer does not exist in the database, this method throws an
    ...
    public void addObserver(Observer observer) {
        ...
        synchronized (mObserverMap) {
            currentObserver = mObserverMap.put(observer, wrapper);
        }
        boolean onAdded = mObservedTableTracker.onAdded(tableIds);
        if (currentObserver == null && onAdded) {
            syncTriggers();
        }
    }
     ...
     *
     * <p>Note that you cannot remove this observer once added. It will be automatically removed
     * when the observer is GC'ed.
     *
     * @param observer The observer to which TableInvalidationTracker will keep a weak reference.
     * @hide
     */
    ...
    volatile boolean inTransaction = false;
            ...
            new Runnable() {
                ...
                public void run() {
                    ...
                    if (invalidatedTableIds != null && !invalidatedTableIds.isEmpty()) {
                        synchronized (mObserverMap) {
                            for (Map.Entry<Observer, ObserverWrapper> entry :
                ...
                private Set<Integer> checkUpdatedTable() {
                    ...
                    Cursor cursor = mDatabase.rawQuery(SELECT_UPDATED_TABLES_SQL, new String[] {});
    ...
    private final Executor queryExecutor = Executors.newSingleThreadExecutor();

    @SuppressWarnings("WeakerAccess")
    public void refreshVersionsAsync() {
        // TODO we should consider doing this sync instead of async.
        if (mPendingRefresh.compareAndSet(false, true)) {
            queryExecutor.execute(mRefreshRunnable);
        }
    }
     ...
     *
     * <p>This can be used for notifying invalidation that cannot be detected by this {@link
     * TableInvalidationTracker}, for example, invalidation from another process.
     *
    ...
    public void notifyObserversByTableNames(String... tables) {
        synchronized (mObserverMap) {
            for (Map.Entry<Observer, ObserverWrapper> entry : mObserverMap.entrySet()) {
    ...
    void syncTriggers(SQLiteDatabase database) {
        ...
        try {
            ...
            // may happen if db is closed. just log.
            Log.e(LOG_TAG, "Cannot run invalidation tracker. Is the db closed?", exception);
        }
    }
     ...
     *
     * <p>It is important that pending trigger changes are applied to the database before any query
     * runs. Otherwise, we may miss some changes.
     *
     * <p>This api should eventually be public.
     */
    void syncTriggers() {
        if (!mDatabase.isOpen()) {
            return;
        }
        syncTriggers(mDatabase);
    }

    /**
     * Wraps an observer and keeps the table information.
     *
     * <p>Internally table ids are used which may change from database to database so the table
    ...
    static class ObserverWrapper {
        ...
        /**
         * Notifies the underlying {@link #mObserver} if it observes any of the specified {@code
         * tables}.
         *
    ...
    public abstract static class Observer {
         ...
         *     multiple tables and you want to know which table is invalidated. This will be names
         *     of underlying tables when you are observing views.
         */
    ...
    static class ObservedTableTracker {
         ...
         * returning any non-null value from getTablesToSync. This allows us to workaround any
         * multi-threaded state syncing issues.
         */
        ...
        /**
         * if getTablesToSync returned non-null, the called should call onSyncCompleted once it is
         * done.
         */
     ...
     *
     * <p>This class will automatically unsubscribe when the wrapped observer goes out of memory.
     */
    static class WeakObserver extends Observer {
        final TableInvalidationTracker mTracker;
        final WeakReference<Observer> mDelegateRef;

        WeakObserver(TableInvalidationTracker tracker, Observer delegate) {
