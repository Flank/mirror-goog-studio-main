 ...
 * suitable for random read access. This allows efficient and trivial implementation of the {@link
 * #mark(int)} and {@link #reset()} functions using file operations.
 *
 * <p>This class deliberately breaks from the {@link InputStream} contract because it is intended
 * for use cases where some of that behavior doesn't make sense. Specifically:
 *
 * <ul>
 *   <li>There is no read limit, and the value passed to {@link #mark(int)} is ignored.
 *   <li>The {@link #reset()} method will only throw an exception if the stream is closed or if
 *       {@link #mark(int)} has never been called. It does <em>not</em> have the concept of an error
 *       from an invalidated read limit, because there is no read limit.
 * </ul>
 */
public class RandomAccessFileInputStream extends InputStream {
     ...
     * access cross the entire file. Equivalent to calling {@link #RandomAccessFileInputStream(File,
     * long, long)} with 0 and {@link File#length()} as the range parameters.
     *
