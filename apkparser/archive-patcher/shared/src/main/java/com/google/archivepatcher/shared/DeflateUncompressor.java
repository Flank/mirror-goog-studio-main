...
public class DeflateUncompressor implements Uncompressor {
    /**
     * Whether to skip the standard zlib header and checksum fields when reading. Defaults to true.
     */
    private boolean nowrap = true;

    /**
     * The size of the buffer used for reading data in during {@link #uncompress(InputStream,
     * OutputStream)}.
     */
    private int inputBufferSize = 32768;

    /**
     * The size of the buffer used for writing data out during {@link #uncompress(InputStream,
     * OutputStream)}.
     */
     ...
     * #uncompress(InputStream, OutputStream)}.
     *
     ...
     * #uncompress(InputStream, OutputStream)}. NB: {@link Inflater} uses an <em>internal</em>
     * buffer and this method adjusts the size of that buffer. This buffer is important for
     * performance, <em>even if the {@link InputStream} is is already buffered</em>.
     *
    ...
    /**
     * Sets whether or not to cache the {@link Inflater} instance. Defaults to false. If set to
     * true, the {@link Inflater} is kept until this object is finalized or until {@link #release()}
     * is called. Instances of {@link Inflater} can be surprisingly expensive, so caching is advised
     * in situations where many resources need to be inflated.
     *
