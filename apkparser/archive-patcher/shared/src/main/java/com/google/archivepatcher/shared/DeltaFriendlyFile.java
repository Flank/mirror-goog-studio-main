...
public class DeltaFriendlyFile {
     ...
     * and the returned list, but the offsets and lengths will be different (the input list
     ...
     *     file order; otherwise, do all the normal work but return null instead of the inverse
     *     ranges
     * @param copyBufferSize the size of the buffer to use for copying bytes between streams
     * @return if <code>generateInverse</code> was true, returns the ranges in the delta-friendly
     *     file that correspond to the ranges in the original file, with identical metadata and in
     *     the same order; otherwise, return null
     * @throws IOException if anything goes wrong
     */
    public static <T> List<TypedRange<T>> generateDeltaFriendlyFile(
