...
public class PartiallyUncompressingPipeTest {
    ...
    public void testWriteAll_Compressed_NoWrapTrue() throws IOException {
        UnitTestZipEntry entry =
                UnitTestZipArchive.makeUnitTestZipEntry("/foo", 7, "frobozz", null);
        stream.pipe(
                new ByteArrayInputStream(entry.getCompressedBinaryContent()),
                Mode.UNCOMPRESS_NOWRAP);
    ...
    public void testWriteAll_Compressed_NoWrapFalse() throws IOException {
        UnitTestZipEntry entry =
                UnitTestZipArchive.makeUnitTestZipEntry("/foo", 6, "frobozz", null);
        ...
        stream.pipe(
                new ByteArrayInputStream(compressBuffer.toByteArray()), Mode.UNCOMPRESS_WRAPPED);
        Assert.assertArrayEquals(entry.getUncompressedBinaryContent(), outBuffer.toByteArray());
    }
    ...
    public void testWriteAll_Multiple() throws IOException {
        ...
        stream.pipe(
                new ByteArrayInputStream(entryA.getCompressedBinaryContent()),
                Mode.UNCOMPRESS_NOWRAP);
        ...
        stream.pipe(
                new ByteArrayInputStream(entryB.getCompressedBinaryContent()),
                Mode.UNCOMPRESS_NOWRAP);
