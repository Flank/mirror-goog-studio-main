...
public class UnitTestZipArchive {
    ...
    /**
     * The data for the fourth entry in the zip file, stored (uncompressed / level 0). Has a
     * comment.
     */
    ...
    /**
     * Makes a unit test entry using the specified parameters <em>plus</em> the corpus from {@link
     * DefaultDeflateCompatibilityWindow#getCorpus()} to provide enough data for an accurate level
     * identification.
     *
     * @param path the file path
     * @param level the level the entry is compressed with
    ...
    public static final UnitTestZipEntry makeUnitTestZipEntry(
        ...
        try {
            corpusText =
                    new String(new DefaultDeflateCompatibilityWindow().getCorpus(), "US-ASCII");
    ...
    static {
        try {
            ...
            throw new RuntimeException(
                    "Core sanity test 1 has failed, unit tests are unreliable", e);
        }
    }
    ...
    /**
     * Verifies the test zip file created by {@link #makeTestZip()} or for sanity, so that the rest
     * of the tests can safely rely upon them. The outputs may be slightly different from platform
     * to platform due to, e.g., filesystem differences that affect the choice of string encoding or
        ...
        for (UnitTestZipEntry testEntry : allEntriesInFileOrder) {
            if (testEntry.path.equals(entry.getName())) {
                if (testEntry.level == 0) {
                    ...
                    Assert.assertEquals(
                            testEntry.getUncompressedBinaryContent().length,
