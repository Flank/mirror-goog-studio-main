...
public class UnitTestZipEntry {
     ...
     * @param comment optional comment, as an ASCII string
     */
    public UnitTestZipEntry(
            String path, int level, boolean nowrap, String content, String comment) {
        this.path = path;
        this.level = level;
        this.nowrap = nowrap;
    ...
    /**
     * Returns the uncompressed content of the entry as a byte array for unit test simplicity. If
     * the level is 0, this is the same as the actual array of bytes that will be present in the zip
    ...
    /**
     * Returns the compressed form of the content, according to the level, that should be found in
     * the zip archive. If the level is 0 (store, i.e. not compressed) this is the same as calling
