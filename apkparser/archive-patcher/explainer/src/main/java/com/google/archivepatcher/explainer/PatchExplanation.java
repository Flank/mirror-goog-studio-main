...
public class PatchExplanation {
    ...
    /**
     * A {@link Comparator} that performs lexical sorting of the path names within {@link
     * EntryExplanation} objects by treating them as UTF-8 strings.
     */
    ...
    /**
     * All entries that are changed (present in both the old and new archive, but different
     * content).
     */
    ...
    /**
     * Returns a read-only view of all entries that are new (only present in the new archive),
     * sorted in ascending order lexicographically by path.
     *
     * @return as described
     */
    ...
    /**
     * Returns a read-only view of all entries that either unchanged or have a zero-cost in the
     * patch (i.e., a copy or rename), sorted in ascending order lexicographically by path.
     *
     * @return as described
     */
    ...
    /**
     * Returns a read-only view of all entries that could have been uncompressed for patching but
     * have been prevented due to resource constraints by a {@link TotalRecompressionLimiter},
     * sorted in ascending order lexicographically by path.
     *
     * @return as described
     */
    ...
    /**
     * Returns the sum total of the sizes of all the entries that are changed in the patch stream.
     * As noted in {@link EntryExplanation#getCompressedSizeInPatch()}, this is an
     * <strong>approximation</strong>.
     *
     * @return as described
     */
    ...
    /**
     * Writes a JSON representation of the data to the specified {@link PrintWriter}. The data has
     ...
     * <br>}
     * </code> <br>
     * Where <code>&lt;entry_list&gt;</code> is a list of zero or more entries of the following
     * form: <code>
    ...
    public void writeJson(PrintWriter writer) {
        ...
        buffer.append("  estimatedResourceConstrainedSize: ")
        ...
        buffer.append(",\n");
        dumpJson(
                getExplainedAsResourceConstrained(),
                "explainedAsResourceConstrained",
                buffer,
                "  ");
        buffer.append("\n");
    ...
    private void dumpJson(
            List<EntryExplanation> explanations,
            String listName,
