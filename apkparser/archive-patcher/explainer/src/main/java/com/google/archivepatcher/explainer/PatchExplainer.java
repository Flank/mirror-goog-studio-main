...
public class PatchExplainer {
     ...
     * compressed patch size estimates and the specified {@link DeltaGenerator} to generate the
     * deltas for the patch.
     *
     ...
     * @param recommendationModifiers optionally, {@link RecommendationModifier}s to use during
    ...
    public List<EntryExplanation> explainPatch(
        ...
        try (TempFileHolder oldTemp = new TempFileHolder();
            ...
            for (QualifiedRecommendation qualifiedRecommendation :
                    plan.getQualifiedRecommendations()) {
                ...
                long oldOffset =
                        qualifiedRecommendation.getOldEntry().getFileOffsetOfCompressedData();
                ...
                long newOffset =
                        qualifiedRecommendation.getNewEntry().getFileOffsetOfCompressedData();
                ...
                try (FileOutputStream deltaOut = new FileOutputStream(deltaTemp.file);
                        BufferedOutputStream bufferedDeltaOut =
                                new BufferedOutputStream(deltaOut)) {
                    deltaGenerator.generateDelta(oldTemp.file, newTemp.file, bufferedDeltaOut);
                    bufferedDeltaOut.flush();
                    long compressedDeltaSize =
                            getCompressedSize(
                                    deltaTemp.file, 0, deltaTemp.file.length(), compressor);
                    result.add(
                            new EntryExplanation(
                                    new ByteArrayHolder(
                                            qualifiedRecommendation
    ...
    private long getCompressedSize(File file, long offset, long length, Compressor compressor)
        ...
        try (OutputStream sink = new NullOutputStream();
                ...
                RandomAccessFileInputStream rafis =
                        new RandomAccessFileInputStream(file, offset, length)) {
