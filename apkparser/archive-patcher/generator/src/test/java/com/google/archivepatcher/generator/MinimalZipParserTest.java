...
public class MinimalZipParserTest {
    ...
    public void testParseEocd() throws IOException {
        ...
        checkExpectedBytes(
                new byte[] {0x50, 0x4b, 0x05, 0x06}, (int) calculatedEndOfCentralDirectory);
        Assert.assertEquals(
    ...
    public void testParseCentralDirectoryEntry() throws Exception {
        ...
        in.reset();
        Assert.assertEquals(
                metadata.getOffsetOfCentralDirectory(),
                in.skip(metadata.getOffsetOfCentralDirectory()));
        ...
        for (UnitTestZipEntry expectedEntry : UnitTestZipArchive.allEntriesInFileOrder) {
            ...
            for (int index = 0; index < 4; index++) {
                byte actualByte =
                        unitTestZipArchive[((int) parsed.getFileOffsetOfLocalEntry()) + index];
