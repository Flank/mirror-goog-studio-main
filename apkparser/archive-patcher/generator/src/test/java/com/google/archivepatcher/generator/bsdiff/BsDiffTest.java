...
public class BsDiffTest {
    ...
    public void searchForMatch() throws Exception {
        ...
        for (String testCase : testCases) {
            ...
            RandomAccessObject suffixArray =
                    new DivSuffixSorter(
                                    new RandomAccessObjectFactory
                                            .RandomAccessByteArrayObjectFactory())
                            .suffixSort(input);
            ...
            for (int lo = 0; lo < size; ++lo) {
                for (int hi = lo + 1; hi <= size; ++hi) {
                    ...
                    RandomAccessObject queryBuf =
                            new RandomAccessObject.RandomAccessByteArrayObject(query);

                    BsDiff.Match match =
                            BsDiff.searchForMatch(suffixArray, input, queryBuf, 0, 0, size);

                    Assert.assertEquals(querySize, match.length);
                    Assert.assertTrue(match.start >= 0);
                    Assert.assertTrue(match.start <= size - match.length);
                    byte[] suffix =
                            Arrays.copyOfRange(bytes, match.start, match.start + match.length);
    ...
    public void generatePatchWithMatcherTest() throws Exception {
...

        {
            ...
            Assert.assertTrue(
                    generatePatchAndCheckCtrlEntries(
                            "aazzzbbb", "aaayyyyybbb", expectedCtrlEntries));
        }
...

        {
            ...
            Assert.assertTrue(
                    generatePatchAndCheckCtrlEntries(
                            "@@012345@ab@de@ghijklm",
                            "#012$45%^&abcdefghijklm",
                            expectedCtrlEntries));
        }
    ...
    private boolean generatePatchAndCheckCtrlEntries(
        ...
        RandomAccessObject oldBytesRo =
                new RandomAccessObject.RandomAccessByteArrayObject(oldBytes);
        RandomAccessObject newBytesRo =
                new RandomAccessObject.RandomAccessByteArrayObject(newBytes);
