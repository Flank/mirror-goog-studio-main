...
public class PreDiffPlannerTest {
    ...
    /**
     * Entry C1 is a small entry WITHOUT the standard corpus of text from {@link
     * DefaultDeflateCompatibilityWindow} appended. It has exactly the same compressed length as
     * {@link #FIXED_LENGTH_ENTRY_C2_LEVEL_6}, and can be used to test the byte-matching logic in
    ...
    /**
     * Entry C2 is a small entry WITHOUT the standard corpus of text from {@link
     * DefaultDeflateCompatibilityWindow} appended. It has exactly the same compressed length as
     * {@link #FIXED_LENGTH_ENTRY_C1_LEVEL_6}, and can be used to test the byte-matching logic in
     * the code when the compressed lengths are identical.
     */
    ...
    private static final UnitTestZipEntry SHADOW_ENTRY_A_LEVEL_1 =
            UnitTestZipArchive.makeUnitTestZipEntry(
                    "/uncompressed data same as A", 1, "entry A", null);
     ...
     */
    private void corruptEntryData(File tempFile, UnitTestZipEntry unitTestEntry)
            throws IOException {
    ...
    private void corruptCompressionMethod(File tempFile, UnitTestZipEntry unitTestEntry)
        ...
        try (RandomAccessFileInputStream rafis = new RandomAccessFileInputStream(tempFile)) {
            ...
            MinimalCentralDirectoryMetadata centralDirectoryMetadata =
                    MinimalZipParser.parseEocd(rafis);
    ...
    private PreDiffPlan invokeGeneratePreDiffPlan(
        ...
        for (DivinationResult divinationResult : diviner.divineDeflateParameters(newFile)) {
            ...
            originalNewArchiveJreDeflateParametersByPath.put(
                    key, divinationResult.divinedParameters);
        }
    ...
    public void testGeneratePreDiffPlan_OneCompressedEntry_LengthsChanged() throws IOException {
        ...
        byte[] oldBytes =
                UnitTestZipArchive.makeTestZip(Collections.singletonList(ENTRY_A_LEVEL_6));
        byte[] newBytes =
                UnitTestZipArchive.makeTestZip(Collections.singletonList(ENTRY_A_LEVEL_9));
        ...
        Assert.assertEquals(
                findRangeWithParams(newFile, ENTRY_A_LEVEL_9),
    ...
    public void testGeneratePreDiffPlan_OneCompressedEntry_BytesChanged() throws IOException {
        ...
        byte[] oldBytes =
                UnitTestZipArchive.makeTestZip(
                        Collections.singletonList(FIXED_LENGTH_ENTRY_C1_LEVEL_6));
        byte[] newBytes =
                UnitTestZipArchive.makeTestZip(
                        Collections.singletonList(FIXED_LENGTH_ENTRY_C2_LEVEL_6));
        File oldFile = storeAndMapArchive(oldBytes);
        File newFile = storeAndMapArchive(newBytes);
        PreDiffPlan plan = invokeGeneratePreDiffPlan(oldFile, newFile);
        Assert.assertNotNull(plan);
    ...
    public void testGeneratePreDiffPlan_OneEntry_UncompressedToCompressed() throws IOException {
        ...
        Assert.assertEquals(1, plan.getNewFileUncompressionPlan().size());
        Assert.assertEquals(
                findRangeWithParams(newFile, ENTRY_A_LEVEL_6),
                plan.getNewFileUncompressionPlan().get(0));
        checkRecommendation(
                plan,
                new QualifiedRecommendation(
    ...
    @Test
    public void testGeneratePreDiffPlan_OneEntry_OldUncompressed_NewNonDeflate()
            throws IOException {
    ...
    @Test
    public void testGeneratePreDiffPlan_OneEntry_OldNonDeflate_NewUncompressed()
            throws IOException {
    ...
    public void testGeneratePreDiffPlan_TwoDifferentEntries_DifferentPaths() throws IOException {
        ...
        byte[] oldBytes =
                UnitTestZipArchive.makeTestZip(Collections.singletonList(ENTRY_A_LEVEL_6));
        byte[] newBytes =
                UnitTestZipArchive.makeTestZip(Collections.singletonList(ENTRY_B_LEVEL_6));
        File oldFile = storeAndMapArchive(oldBytes);
        File newFile = storeAndMapArchive(newBytes);
        PreDiffPlan plan = invokeGeneratePreDiffPlan(oldFile, newFile);
        Assert.assertNotNull(plan);
    ...
    public void testGeneratePreDiffPlan_TwoEntriesEachArchive_SwappingOrder() throws IOException {
        ...
        Assert.assertEquals(
                findRangeWithParams(newFile, ENTRY_B_LEVEL_9),
                plan.getNewFileUncompressionPlan().get(0));
        Assert.assertEquals(
                findRangeWithParams(newFile, ENTRY_A_LEVEL_9),
