...
public class MinimalZipArchive {
    ...
    /**
     * Sorts {@link MinimalZipEntry} objects by {@link MinimalZipEntry#getFileOffsetOfLocalEntry()}
     * in ascending order.
     */
    private static final Comparator<MinimalZipEntry> LOCAL_ENTRY_OFFSET_COMAPRATOR =
            new Comparator<MinimalZipEntry>() {
                ...
                public int compare(MinimalZipEntry o1, MinimalZipEntry o2) {
                    return Long.compare(
                            o1.getFileOffsetOfLocalEntry(), o2.getFileOffsetOfLocalEntry());
                }
            };

    /**
     * Generate a listing of all of the files in a zip archive in file order and return it. Each
     * entry is a {@link MinimalZipEntry}, which has just enough information to generate a patch.
     *
    ...
    private static List<MinimalZipEntry> listEntriesInternal(RandomAccessFileInputStream in)
        ...
        for (int x = 0; x < minimalZipEntries.size(); x++) {
            ...
            long relativeDataOffset =
                    MinimalZipParser.parseLocalEntryAndGetCompressedDataOffset(in);
            entry.setFileOffsetOfCompressedData(
                    entry.getFileOffsetOfLocalEntry() + relativeDataOffset);
        }
