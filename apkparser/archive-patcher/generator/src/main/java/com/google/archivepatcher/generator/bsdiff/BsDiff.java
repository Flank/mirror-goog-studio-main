 ...
 *
 * <p>A canonical description of the bsdiff algorithm can be found at the following URL:
 * http://www.daemonology.net/bsdiff/
 *
 * <p>Since Java only supports "int" for array indexing, the maximum size of files that this
...
class BsDiff {
     ...
     * @return the number of matching bytes in the two arrays starting at the specified indices;
     *     zero if the first byte fails to match
     */
    ...
    static int lengthOfMatch(
            ...
            throws IOException {
        final int max =
                Math.min((int) oldData.length() - oldStart, (int) newData.length() - newStart);
        if (max > 0) {
    ...
    static Match searchForMatch(
        ...
        if (BsUtil.lexicographicalCompare(
            ...
            return searchForMatch(
                    groupArray, oldData, newData, newStart, pivot, oldDataRangeStartB);
        }
