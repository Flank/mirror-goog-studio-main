...
class PreDiffPlanner {
    ...
    /**
     * Optional {@link RecommendationModifier}s that will be applied after the default
     * recommendations have been made but before the {@link PreDiffPlan} is constructed.
     */
     ...
     * @param newArchiveJreDeflateParametersByPath the {@link JreDeflateParameters} for each entry
     *     in the new archive, with paths as keys
     * @param recommendationModifiers optionally, {@link RecommendationModifier}s to be applied
     *     after the default recommendations have been made but before the {@link PreDiffPlan} is
     *     generated in {@link #generatePreDiffPlan()}.
     */
    PreDiffPlanner(
            File oldFile,
    ...
    PreDiffPlan generatePreDiffPlan() throws IOException {
        ...
        for (RecommendationModifier modifier : recommendationModifiers) {
            ...
            recommendations =
                    modifier.getModifiedRecommendations(oldFile, newFile, recommendations);
        }
            ...
            if (recommendation.getRecommendation().uncompressNewEntry) {
                ...
                TypedRange<JreDeflateParameters> range =
                        new TypedRange<JreDeflateParameters>(
                                offset, length, newJreDeflateParameters);
     ...
     * @throws IOException if there are any problems reading the input files
     */
    private QualifiedRecommendation getRecommendation(
            MinimalZipEntry oldEntry, MinimalZipEntry newEntry) throws IOException {
    ...
    /**
     * Returns true if the entries are unsuitable for doing an uncompressed diff. This method
     * returns true if either of the entries is compressed in an unsupported way (a non-deflate
     * compression algorithm) or if the new entry is compressed in a supported but unreproducible
     * way.
     *
     * @param oldEntry the entry in the old archive
     * @param newEntry the entry in the new archive
     ...
     * returns true if both of the entries are already uncompressed, i.e. are already in the best
     * form for diffing.
     *
     * @param oldEntry the entry in the old archive
     * @param newEntry the entry in the new archive
    ...
    /**
     * Returns true if the entry is uncompressed in the old archive and compressed in the new
     * archive. This method does not check whether or not the compression is reproducible. It is
     * assumed that any compressed entries encountered are reproducibly compressed.
     *
     * @param oldEntry the entry in the old archive
     * @param newEntry the entry in the new archive
     * @return as described
    ...
    /**
     * Returns true if the entry is compressed in the old archive and uncompressed in the new
     * archive. This method does not check whether or not the compression is reproducible because
     * that information is irrelevant to this decision (it does not matter whether the compression
     * in the old archive is reproducible or not, because that data does not need to be recompressed
     * at patch apply time).
     *
     * @param oldEntry the entry in the old archive
     * @param newEntry the entry in the new archive
    ...
    private boolean compressedBytesChanged(MinimalZipEntry oldEntry, MinimalZipEntry newEntry)
        ...
        try (RandomAccessFileInputStream oldRafis =
                ...
                RandomAccessFileInputStream newRafis =
                        new RandomAccessFileInputStream(
                                newFile,
                                newEntry.getFileOffsetOfCompressedData(),
                                newEntry.getCompressedSize());
