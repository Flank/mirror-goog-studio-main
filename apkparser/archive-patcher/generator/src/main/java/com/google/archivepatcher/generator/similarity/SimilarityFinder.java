...
public abstract class SimilarityFinder {
    ...
    /**
     * Create a new instance to check for similarity of arbitrary files against the specified
     * entries in the specified archive.
     *
     ...
     * @return a {@link List} of {@link MinimalZipEntry} entries (possibly empty but never null)
     *     from the base archive that are similar to the new archive; if the list has more than one
     *     entry, the entries should be in order from most similar to least similar.
     */
    public abstract List<MinimalZipEntry> findSimilarFiles(
            File newArchive, MinimalZipEntry newEntry);
}
