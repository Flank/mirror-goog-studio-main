 ...
 *
 * <p>The offsets, lengths and fields that this parser understands and exposes are based on version
 ...
 *
 * <ul>
 *   <li>https://www.pkware.com/documents/APPNOTE/APPNOTE-6.3.3.TXT
 * </ul>
 *
 * <p>Please note that the parser does not attempt to verify the version-needed-to-extract field,
 * since there is no guarantee that all ZIP implementations have set the value correctly to the
 * minimum needed to truly support extraction.
 */
class MinimalZipParser {
    ...
    /**
     * Standard 32-bit signature for a "central directory entry" record in a ZIP-like archive. This
     * is in little-endian order.
     */
    ...
    /**
     * Parses one local file entry and returns the offset from the first byte at which the
     ...
     * @throws IOException if unable to complete the parsing
     */
    public static long parseLocalEntryAndGetCompressedDataOffset(InputStream in)
            throws IOException {
     ...
     * @return the offset in the file at which the first byte of the EOCD signature is located, or
     *     -1 if the signature is not found in the search buffer
    ...
    /**
     * Find the end-of-central-directory record by scanning backwards looking for the signature of
     * the record.
     *
     ...
     * @return the metadata
     * @throws IOException if unable to read
     * @throws ZipException if the metadata indicates this is a zip64 archive, which is not
     *     supported
     */
