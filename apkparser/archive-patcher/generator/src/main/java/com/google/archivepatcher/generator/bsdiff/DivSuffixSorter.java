 ...
 *
 * <p>Straightforward reimplementation of the divsufsort algorithm given in:
 *
 * <pre><code>
...
public final class DivSuffixSorter implements SuffixSorter {
    ...
    private static final int[] SQQ_TABLE = {
        ...
        144, 144, 145, 146, 147, 148, 149, 150, 150, 151, 152, 153, 154, 155, 155, 156, 157, 158,
        159, 160, 160, 161, 162, 163, 163, 164, 165, 166, 167, 167, 168, 169, 170, 170, 171, 172,
        173, 173, 174, 175, 176, 176, 177, 178, 178, 179, 180, 181, 181, 182, 183, 183, 184, 185,
        185, 186, 187, 187, 188, 189, 189, 190, 191, 192, 192, 193, 193, 194, 195, 195, 196, 197,
        197, 198, 199, 199, 200, 201, 201, 202, 203, 203, 204, 204, 205, 206, 206, 207, 208, 208,
        209, 209, 210, 211, 211, 212, 212, 213, 214, 214, 215, 215, 216, 217, 217, 218, 218, 219,
        219, 220, 221, 221, 222, 222, 223, 224, 224, 225, 225, 226, 226, 227, 227, 228, 229, 229,
        230, 230, 231, 231, 232, 232, 233, 234, 234, 235, 235, 236, 236, 237, 237, 238, 238, 239,
    ...
    @Override
    public RandomAccessObject suffixSort(RandomAccessObject input)
    ...
    private final void constructSuffixArray(int[] bucketA, int[] bucketB, int n, int m)
        ...
        if (0 < m) {
            ...
            for (c1 = ALPHABET_SIZE - 2; 0 <= c1; --c1) {
                ...
                for (i = bucketB[(c1) * ALPHABET_SIZE + (c1 + 1)],
                                j = bucketA[c1 + 1] - 1,
                                k = 0,
                                c2 = -1;
                        i <= j;
        ...
        if (0 < m) {
            ...
            for (i = n - 1, j = m, c0 = readInput(n - 1); 0 <= i; ) {
                ...
                if (0 <= i) {
                    ...
                    for (--i, c1 = c0; (0 <= i) && ((c0 = readInput(i)) <= c1); --i, c1 = c0) {}
                    writeSuffixArray(
                            readSuffixArray(ISAb + --j), ((t == 0) || (1 < (t - i))) ? t : ~t);
                }
            }
            ...
            for (c0 = ALPHABET_SIZE - 2, k = m - 1; 0 <= c0; --c0) {
                ...
                bucketB[(c0) * ALPHABET_SIZE + (c0 + 1)] =
                        i - bucketB[(c0) * ALPHABET_SIZE + (c0)] + 1;
    ...
    /**
     * special version of ssCompare for handling <code>ssCompare(T, &(PAi[0]), PA + *a, depth)
     * </code> situation.
     */
    ...
    private final int ssCompare(int p1, int p2, int depth) throws IOException {
        ...
        for (U1 = depth + readSuffixArray(p1),
                        U2 = depth + readSuffixArray(p2),
                        U1n = readSuffixArray(p1 + 1) + 2,
                        U2n = readSuffixArray(p2 + 1) + 2;
                (U1 < U1n) && (U2 < U2n) && (readInput(U1) == readInput(U2));
                ++U1, ++U2) {}
    ...
    private final void ssSwapMerge(
            int PA, int first, int middle, int last, int buf, int bufsize, int depth)
            throws IOException {
        ...
        for (check = 0, ssize = 0; ; ) {
            ...
            if ((last - middle) <= bufsize) {
                ...
                if (((check & 1) != 0)
                        || (((check & 2) != 0)
                                && (ssCompare(
                                                PA + getIDX(readSuffixArray(first - 1)),
                                                PA + readSuffixArray(first),
                                                depth)
                                        == 0))) {
                ...
                if (((check & 4) != 0)
                        && ((ssCompare(
                                        PA + getIDX(readSuffixArray(last - 1)),
                                        PA + readSuffixArray(last),
                                        depth)
                                == 0))) {
            ...
            if (0 < m) {
                ...
                if (ssCompare(
                                PA + getIDX(readSuffixArray(middle - 1)),
                                PA + readSuffixArray(middle),
                                depth)
                        == 0) {
    ...
    private final void ssMintroSort(int PA, int first, int last, int depth) throws IOException {
        ...
        for (ssize = 0, limit = ssIlg(last - first); ; ) {
            ...
            if (limit < 0) {
                for (a = first + 1,
                                v = readInput(Td + readSuffixArray(PA + readSuffixArray(first)));
                        a < last;
                        ++a) {
            ...
            for (b = first;
                    (++b < last)
                            && ((x = readInput(Td + readSuffixArray(PA + readSuffixArray(b))))
                                    == v);
                    ) {}
            if (((a = b) < last) && (x < v)) {
                for (;
            ...
            for (c = last;
                    (b < --c)
                            && ((x = readInput(Td + readSuffixArray(PA + readSuffixArray(c))))
                                    == v);
                    ) {}
            if ((b < (d = c)) && (x > v)) {
                for (;
    ...
    private final TRPartitionResult trPartition(int ISAd, int first, int middle, int last, int v)
            throws IOException {
        int a, b, c, d, e, f; // ptr
        int t, s, x = 0;
            ...
            if (limit < 0) {
                if (limit == -1) {
                    ...
                    if ((a - first) <= (last - b)) {
                        if (1 < (a - first)) {
                            stack[ssize++] =
                                    new StackElement(ISAd, b, last, trIlg(last - b), trlink);
                            last = a;
                            limit = trIlg(a - first);
                        } else if (1 < (last - b)) {
                            first = b;
                        ...
                        if (1 < (last - b)) {
                            stack[ssize++] =
                                    new StackElement(ISAd, first, a, trIlg(a - first), trlink);
                            first = b;
                            limit = trIlg(last - b);
                        } else if (1 < (a - first)) {
                            last = a;
                    ...
                    if (first < last) {
                        ...
                        if (budget.check(a - first) != 0) {
                            if ((a - first) <= (last - a)) {
                                ...
                                if (1 < (last - a)) {
                                    stack[ssize++] =
                                            new StackElement(ISAd + incr, first, a, next, trlink);
                                    first = a;
                                    limit = -3;
                                } else {
                                    ISAd += incr;
    ...
    private final int trMedian5(int ISAd, int v1, int v2, int v3, int v4, int v5)
            throws IOException {
        int t;
        if (readSuffixArray(ISAd + readSuffixArray(v2))
                > readSuffixArray(ISAd + readSuffixArray(v3))) {
            t = v2;
            v2 = v3;
            v3 = t;
        }
        if (readSuffixArray(ISAd + readSuffixArray(v4))
                > readSuffixArray(ISAd + readSuffixArray(v5))) {
            t = v4;
            v4 = v5;
            v5 = t;
        }
        if (readSuffixArray(ISAd + readSuffixArray(v2))
                > readSuffixArray(ISAd + readSuffixArray(v4))) {
            t = v2;
            v2 = v4;
            v4 = t;
            t = v3;
        ...
        if (readSuffixArray(ISAd + readSuffixArray(v3))
                > readSuffixArray(ISAd + readSuffixArray(v4))) {
            return v4;
        }
        return v3;
    ...
    private final int trMedian3(int ISAd, int v1, int v2, int v3) throws IOException {
        if (readSuffixArray(ISAd + readSuffixArray(v1))
                > readSuffixArray(ISAd + readSuffixArray(v2))) {
            int t = v1;
            v1 = v2;
            v2 = t;
        }
    ...
    private final void trInsertionSort(int ISAd, int first, int last) throws IOException {
        ...
        for (a = first + 1; a < last; ++a) {
            for (t = readSuffixArray(a), b = a - 1;
                    0
                            > (r =
                                    readSuffixArray(ISAd + t)
                                            - readSuffixArray(ISAd + readSuffixArray(b)));
                    ) {
                do {
