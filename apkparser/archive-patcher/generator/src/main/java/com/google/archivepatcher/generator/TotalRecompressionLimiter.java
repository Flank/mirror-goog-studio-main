...
public class TotalRecompressionLimiter implements RecommendationModifier {
    ...
    /**
     * Create a new limiter that will restrict the total number of bytes that need to be
     * recompressed to the specified quantity.
     *
    ...
    private static class UncompressedNewEntrySizeComparator
        ...
        public int compare(QualifiedRecommendation qr1, QualifiedRecommendation qr2) {
            return Long.compare(
                    qr1.getNewEntry().getUncompressedSize(),
                    qr2.getNewEntry().getUncompressedSize());
        }
    }
}
