...
class BsDiffMatcher implements Matcher {
    ...
    /**
     * Contains order of the sorted suffixes of |oldData|. The element at mGroupArray[i] contains
     * the position of oldData[i ... oldData.length - 1] in the sorted list of suffixes of
     * |oldData|.
     */
    private final RandomAccessObject mGroupArray;

    /**
     * The index in |oldData| of the first byte of the match. Zero if no matches have been found
     * yet.
     */
    private int mOldPos;

    /**
     * The index in |newData| of the first byte of the match. Zero if no matches have been found
     * yet. The next match will be searched starting at |mNewPos| + |mMatchLen|.
     */
    private int mNewPos;
    ...
    /**
     * A limit on how many total match lengths encountered, to exit the match extension loop in
     * next() and prevent O(n^2) behavior.
     */
        ...
        while (mNewPos < newData.length()) {
            ...
            BsDiff.Match match =
                    BsDiff.searchForMatch(
                            mGroupArray, oldData, newData, mNewPos, 0, (int) oldData.length());
            mOldPos = match.start;
            ...
            if (mMatchLen > numMatches + mMinimumMatchLength
                    || totalMatchLen >= mTotalMatchLenBudget) {
