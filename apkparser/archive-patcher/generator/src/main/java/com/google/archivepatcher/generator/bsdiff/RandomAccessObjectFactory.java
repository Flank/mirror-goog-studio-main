...
public interface RandomAccessObjectFactory {
    ...
    public static final class RandomAccessFileObjectFactory implements RandomAccessObjectFactory {
         ...
         *
         * @param mode the file mode string ("r", "w", "rw", etc - see documentation for {@link
         *     RandomAccessFile})
         */
         ...
         * RandomAccessFileObject} representing the new temp file. The temp file does not need to
         * explicitly be managed (deleted) by the caller, as long as the caller ensures {@link
         * RandomAccessObject#close()} is called when the object is no longer needed.
         */
        // TODO(hartmanng): rethink the handling of these temp files. It's confusing and shouldn't
        // really be the responsibility of RandomAccessObject.
        @Override
    ...
    public static final class RandomAccessMmapObjectFactory implements RandomAccessObjectFactory {
        ...
        /**
         * Creates a temp file, and returns a {@link RandomAccessFile} wrapped in a {@link
         * RandomAccessMmapObject} representing the new temp file. The temp file does not need to
         * explicitly be managed (deleted) by the caller, as long as the caller ensures {@link
         * RandomAccessObject#close()} is called when the object is no longer needed.
         */
        // TODO(hartmanng): rethink the handling of these temp files. It's confusing and shouldn't
