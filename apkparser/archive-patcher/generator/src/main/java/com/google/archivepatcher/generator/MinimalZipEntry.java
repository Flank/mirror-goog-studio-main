...
public class MinimalZipEntry {
    ...
    /**
     * The file offset at which the first byte of the data for the entry begins. For compressed
     * data, this is the first byte of the deflated data; for uncompressed data, this is the first
     * byte of the uncompressed data.
     */
     ...
     *     nominally controls the default character encoding for file names and comments; see {@link
     *     #getFileName()} for more information on encoding
     ...
     * size of the compressed data; for uncompressed entries, this is the same as {@link
     * #getUncompressedSize()}.
     *
     * @return as described
     */
     ...
     * purpose flag was set to 1, these bytes should be encoded with the UTF8 character set;
     * otherwise the character set should be Cp437. Adherence to this standard varies significantly,
     * and some systems use the default character set for the environment instead of Cp437 when
     * writing these bytes. For such instances, callers can obtain the raw bytes by using {@link
     * #getFileNameBytes()} instead and checking the value of the 11th bit of the general purpose
     * bit flag for a hint using {@link #getGeneralPurposeFlagBit11()}. There is also something
     * called EFS ("0x0008 extra field storage") that specifies additional behavior for character
     * encoding, but this tool doesn't support it as the use is not standardized.
     *
     * @return as described
     */
    ...
    public int hashCode() {
        ...
        result =
                prime * result + (int) (crc32OfUncompressedData ^ (crc32OfUncompressedData >>> 32));
        result = prime * result + Arrays.hashCode(fileNameBytes);
        result =
                prime * result
                        + (int) (fileOffsetOfCompressedData ^ (fileOffsetOfCompressedData >>> 32));
