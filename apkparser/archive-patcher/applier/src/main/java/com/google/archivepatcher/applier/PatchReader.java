...
public class PatchReader {
    ...
    /**
     * Reads patch data from the specified {@link InputStream} up to but not including the first
     * byte of delta bytes, and returns a {@link PatchApplyPlan} that describes all the operations
     * that need to be performed in order to apply the patch. When this method returns, the stream
     * is positioned so that the next read will be the first byte of delta bytes corresponding to
     * the first {@link DeltaDescriptor} in the returned plan.
     *
    ...
    public PatchApplyPlan readPatchApplyPlan(InputStream in) throws IOException {
        ...
        long deltaFriendlyOldFileSize =
                checkNonNegative(dataIn.readLong(), "delta-friendly old file size");

        // Read old file uncompression instructions.
        int numOldFileUncompressionInstructions =
                (int)
                        checkNonNegative(
        ...
        for (int x = 0; x < numOldFileUncompressionInstructions; x++) {
            long offset =
                    checkNonNegative(dataIn.readLong(), "old file uncompression range offset");
            long length =
                    checkNonNegative(dataIn.readLong(), "old file uncompression range length");
            ...
            lastReadOffset =
                    offset + length; // To check that the next range starts after the current one
        }
        ...
        for (int x = 0; x < numDeltaFriendlyNewFileRecompressionInstructions; x++) {
            long offset =
                    checkNonNegative(
                            dataIn.readLong(),
                            "delta-friendly new file recompression range offset");
            long length =
                    checkNonNegative(
                            dataIn.readLong(),
                            "delta-friendly new file recompression range length");
            if (offset < lastReadOffset) {
                throw new PatchFormatException(
            ...
            int strategy =
                    (int) checkRange(dataIn.readUnsignedByte(), 0, 2, "recompression strategy");
            int nowrapInt =
                    (int) checkRange(dataIn.readUnsignedByte(), 0, 1, "recompression nowrap");
            TypedRange<JreDeflateParameters> range =
                    new TypedRange<JreDeflateParameters>(
                            offset,
                            length,
                            JreDeflateParameters.of(
                                    level, strategy, nowrapInt == 0 ? false : true));
        ...
        for (int x = 0; x < numDeltaRecords; x++) {
            ...
            DeltaDescriptor descriptor =
                    new DeltaDescriptor(
                            ...
                            new TypedRange<Void>(
                                    deltaFriendlyOldFileWorkRangeOffset,
                                    deltaFriendlyOldFileWorkRangeLength,
                                    null),
                            new TypedRange<Void>(
                                    deltaFriendlyNewFileWorkRangeOffset,
