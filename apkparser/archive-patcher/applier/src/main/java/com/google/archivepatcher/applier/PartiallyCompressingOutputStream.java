...
public class PartiallyCompressingOutputStream extends FilterOutputStream {
     ...
     * compressing bytes, but never both. All manipulation of the compression state machinery is
     * done within this method. When the end of a compression range is reached it is completely
     * flushed to the output stream, to keep things as straightforward as possible.
     *
    ...
    private long bytesTillCompressionEnds() {
        ...
        return (nextCompressedRange.getOffset() + nextCompressedRange.getLength())
                - numBytesWritten;
    }
}
