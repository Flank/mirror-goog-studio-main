 ...
 *
 * <p>The plan for recompressing the delta-friendly new file is a {@link List} of {@link TypedRange}
 ...
 * produced by executing this plan is the new file.
 *
 * <p>The "plan" for the deltas themselves is a {@link List} of {@link DeltaDescriptor} entries that
 * describe the deltas present in the patch stream. Nominally, a {@link PatchReader} is used to read
 * the stream up to the first byte of the deltas; the plan for the deltas is ordered in the same
 * order as the patch stream and contains the byte length of each delta, so it is then trivial to
 * read each delta in order and apply it.
 */
public class PatchApplyPlan {
    ...
    /**
     * The expected size of the delta-friendly old file after executing the {@link
     * #oldFileUncompressionPlan}.
     */
     ...
     * @param deltaFriendlyNewFileRecompressionPlan the plan for recompressing the delta-friendly
     ...
     * {@link #getOldFileUncompressionPlan()}. This can be used to pre-allocate the necessary space
     * to hold the delta-friendly old file.
     *
     * @return as described
     */
    ...
    /**
     * Returns the delta descriptors that describe how and what to do to the delta-friendly old
     * file.
     *
     * @return as described
     */
