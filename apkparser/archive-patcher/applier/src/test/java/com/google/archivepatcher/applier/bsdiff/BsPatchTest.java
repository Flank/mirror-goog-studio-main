...
public class BsPatchTest {
     ...
     * the argument to a RandomAccessFile constructor... but the old file is a resource loaded at
     * test run-time, potentially from a JAR, and therefore a copy must be made in the filesystem to
     * access via RandomAccessFile. This is not true for the new file or the patch file, both of
     * which are streamable.
     */
    ...
    public void testTransformBytes_Error_NotEnoughBytes() throws IOException {
        ...
        try {
            BsPatch.transformBytes(
                    2, patchIn, oldData, new ByteArrayOutputStream(), buffer1, buffer2);
