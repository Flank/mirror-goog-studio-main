...
public class PatchReaderTest {
    ...
    private static final List<TypedRange<Void>> OLD_DELTA_FRIENDLY_UNCOMPRESS_PLAN =
            Collections.unmodifiableList(
                    Arrays.asList(
                            OLD_DELTA_FRIENDLY_UNCOMPRESS_RANGE1,
                            OLD_DELTA_FRIENDLY_UNCOMPRESS_RANGE2));
    ...
    private byte[] writeTestPatch() throws IOException {
        ...
        for (TypedRange<Void> range : oldDeltaFriendlyUncompressPlan) {
            patchOut.writeLong(
                    corruption.corruptOldFileUncompressionInstructionOffset
                            ? -1
                            : range.getOffset());
            patchOut.writeLong(
                    corruption.corruptOldFileUncompressionInstructionLength
                            ? -1
                            : range.getLength());
        }
