...
import static com.android.ddmlib.adbserver.DeviceState.HostConnectionType.LOCAL;
import static com.android.ddmlib.adbserver.DeviceState.HostConnectionType.USB;
        ...
        switch (splitPayload[0]) {
            case "host":
                ...
                if (!NON_WILDCARD_TRANSPORT_DEVICE_COMMANDS.contains(command)) {
                    device = findAnyDeviceWithProtocol(Arrays.asList(LOCAL, USB));
                    if (device == null) {
                        return null;
                    }
                    ...
                    switch (command) {
                        case "transport":
                            ...
                            break;
                        case "transport-usb":
                            device = findAnyDeviceWithProtocol(Collections.singletonList(USB));
                            break;
                        case "transport-local":
                            device = findAnyDeviceWithProtocol(Collections.singletonList(LOCAL));
                            break;
                        case "transport-any":
                            device = findAnyDeviceWithProtocol(Arrays.asList(LOCAL, USB));
                            break;
