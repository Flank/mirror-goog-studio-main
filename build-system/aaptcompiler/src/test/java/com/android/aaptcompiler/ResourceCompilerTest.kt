...
class ResourceCompilerTest {
  ...
  private fun testPngFile(type: AaptResourceType, input: String, config: String = ""): File {
    val resourceFolder = tempFolder.newFolder(type.tagName)
    val configSuffix = if (config.isEmpty()) "" else "-$config"
  ...
  @Test
  fun testCompileRawFileNotSupported() {
    val input = """
      I'm just writing some text here, because raw files can be anything.
    """.trimIndent()

    try {
      testRawFile(input, "txt")
      fail()
    } catch (e: IllegalStateException) {
      Truth.assertThat(e.message).contains("Raw")
      Truth.assertThat(e.message).contains("support not yet implemented")
      // expected
    }
  }
  ...
  @Test
  fun testCompilePngNotSupported() {
    val input = """
    ...
    try {
      testPngFile(AaptResourceType.LAYOUT, input)
      fail()
    } catch (e: IllegalStateException) {
      Truth.assertThat(e.message).contains("PNG")
      Truth.assertThat(e.message).contains("support not yet implemented")
      // expected
    }
  }
