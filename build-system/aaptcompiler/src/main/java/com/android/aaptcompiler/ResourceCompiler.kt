...
class ResourceCompiler(
   ...
   */
  fun compileResource(file: File, outputDirectory: File) {
    // Skip hidden files.
    if (file.isHidden) {
    ...
    // TODO(b/139297538): add compile options.
    compileFunction(pathData, outputDirectory)
  }

  private fun getCompileMethod(pathData: ResourcePathData): (ResourcePathData, File) -> Unit {
   ...
   */
  private fun compileTable(pathData: ResourcePathData, outputDirectory: File) {
    ...
    // TODO(b/142961412): parser options
    val options = TableExtractorOptions()
    val tableExtractor = TableExtractor(table, pathData.source, pathData.config, options)

    if (!tableExtractor.extract(pathData.file.inputStream())) {
   ...
   * @param outputDirectory the directory to which the compiled file is to be placed.
   */
  private fun compileFile(pathData: ResourcePathData, outputDirectory: File) {
    val outputFile = File(outputDirectory, pathData.getIntermediateContainerFilename())
   ...
   * @throws IllegalStateException A failure occurred in processing this resource.
   */
  private fun compileXml(pathData: ResourcePathData, outputDirectory: File) {
    val outputFile = File(outputDirectory, pathData.getIntermediateContainerFilename())
   ...
   */
  private fun compilePng(pathData: ResourcePathData, outputDirectory: File) {
    val outputFile = File(outputDirectory, pathData.getIntermediateContainerFilename())
