...
public class KeymaestroHybridEncrypter {
     ...
     * @return The encrypted ciphertext formatted as a zero byte, followed by the 4-byte key
     *     identity, followed by the per-message 64-byte P256 point, followed by a random 12-byte
     *     AES-GCM nonce, followed by the AES-256-GCM ciphertext, followed by a 16-byte AES-GCM tag.
    ...
    /**
     * Performs an elliptic-curve Diffie-Hellmen exchange, using P256. Package-visibility for
     * testing.
     */
    static byte[] ecdh(ECPrivateKey privateKey, ECPoint publicPoint)
            throws GeneralSecurityException {
        ...
        for (int i = 0; i < blocks; i++) {
            ...
            if (i < blocks - 1) {
                ...
                System.arraycopy(
                        ...
                        result,
                        i * digestLength,
                        extractBytes - (blocks - 1) * digestLength);
            }
        }
        return result;
    }
     ...
     * where each point is serialized as a 32-byte big-endian integer. Package-visibility for
     * testing.
     */
    ...
    private static ECPoint deserializePoint(byte[] serializedPoint, int offset, int length)
        ...
        BigInteger x =
                new BigInteger(
                        1 /* positive */,
                        Arrays.copyOfRange(serializedPoint, offset, offset + FIELD_LENGTH));
        BigInteger y =
                new BigInteger(
                        1 /* positive */,
                        Arrays.copyOfRange(
                                serializedPoint, offset + FIELD_LENGTH, offset + 2 * FIELD_LENGTH));
