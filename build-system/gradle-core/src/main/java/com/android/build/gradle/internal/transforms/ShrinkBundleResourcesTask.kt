...
open class ShrinkBundleResourcesTask : AndroidVariantTask() {
    ...
    class ConfigAction(val scope: VariantScope) : TaskConfigAction<ShrinkBundleResourcesTask> {
        override fun getName() = scope.getTaskName("shrink", "Resources")
        override fun getType() = ShrinkBundleResourcesTask::class.java

        override fun execute(task: ShrinkBundleResourcesTask) {
            task.compressedResourceFile = scope.artifacts
                .appendArtifact(
