...
/**
 * Utilities to use transforms with the [com.android.builder.merge.IncrementalFileMerger].
 */
object IncrementalFileMergerTransformUtils {
    ...
    fun toIncrementalInput(
        ...
        if (contentMap != null) {
            contentMap[input] = directoryInput
        }
    ...
    fun toNonIncrementalInput(
        ...
        for (jarInput in transformInput.jarInputs) {
            val mergeInput = toNonIncrementalInput(jarInput, zipCache, cacheUpdates, contentMap)
            if (mergeInput != null) {
        ...
        for (dirInput in transformInput.directoryInputs) {
            val mergeInput = toNonIncrementalInput(dirInput, contentMap)
            if (mergeInput != null) {
                builder.add(mergeInput)
            }
        }
    ...
    private fun mapStatus(status: Status): FileStatus? {
        when (status) {
            Status.ADDED -> return FileStatus.NEW
            Status.CHANGED -> return FileStatus.CHANGED
            Status.NOTCHANGED -> return null
            Status.REMOVED -> return FileStatus.REMOVED
            else -> throw AssertionError()
        }
    }
}
