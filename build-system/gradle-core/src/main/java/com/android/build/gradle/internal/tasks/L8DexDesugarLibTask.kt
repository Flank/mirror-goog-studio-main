...
abstract class L8DexDesugarLibTask : NonIncrementalTask() {
    ...
    abstract val keepRules: ConfigurableFileCollection
    ...
    class CreationAction(
        ...
        override fun configure(task: L8DexDesugarLibTask) {
            ...
            task.minSdkVersion.set(
                variantScope.variantConfiguration.minSdkVersionWithTargetDeviceApi.apiLevel)
            ...
            task.keepRules.from(variantScope.globalScope.project.files(
                subProjectKeepRules,
...
class L8DexRunnable @Inject constructor(val params: L8DexParams) : Runnable {
    override fun run() {
        ...
        runL8(
            ...
            getAllFilesUnderDirectories(params.keepRules).map { it.toPath() })
    }

    private fun getAllFilesUnderDirectories(dirs: Set<File>) : List<File> {
