...
class TimingEnvironmentTest {
    ...
    private fun timeTest(action : () -> Unit) : String {
        val folder = temporaryFolder.newFolder()
        lateinit var file : File
        TimingEnvironment(folder, "test").use {
            action()
    ...
    fun `untracked gap at start of scope is logged`() {
        val log = timeTest {
            Thread.sleep(50) // <- This time is outside of "scope" below
            time("scope") {
                Thread.sleep(50)
            }
        }
    ...
    fun `untracked gap at end of scope is logged`() {
        val log = timeTest {
            ...
            Thread.sleep(50) // <- This time is outside of "scope" abovr
        }
        assertThat(replaceTimes(log)).isEqualTo("""
            # C/C++ build system timings
            test
    ...
    fun `untracked gap between two scopes is logged`() {
        val log = timeTest {
            ...
            Thread.sleep(50) // <- This time is between of "scope-1" and "scope-2"
            time("scope-2") {
                Thread.sleep(50)
            }
        }
