...
public final class InstantRunContentProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        if (isMainProcess()) {
            Log.i(LOG_TAG, "starting instant run server: is main process");
            Server.create(getContext());
        } else {
            Log.i(LOG_TAG, "not starting instant run server: not main process");
        }
        return true;
    }
    ...
    private boolean isMainProcess() {
        ...
        if (AppInfo.applicationId != null) {
            ...
            if (!isMainProcess && !foundPackage) {
                ...
                isMainProcess = true;
                Log.w(LOG_TAG, "considering this process main process:" +
