...
# Run Bazel
"${script_dir}/bazel" --max_idle_secs=60 test --keep_going --config=local --config=cloud_resultstore --build_tag_filters=-no_mac --test_tag_filters=-no_mac,-no_test_mac,-qa_sanity,-qa_fast,-qa_unreliable,-perfgate_only --profile=${dist_dir}/prof -- $(< "${script_dir}/targets")
