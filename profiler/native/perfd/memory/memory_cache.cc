...
bool MemoryCache::StartHeapDumpSample(const MemoryData::HeapDumpSample& sample) {
  ...
  put_heap_dump_sample_index_ = IncrementSampleIndex(put_heap_dump_sample_index_);
...
bool MemoryCache::EndHeapDumpSample(int64_t end_time, bool success) {
  ...
  if (last_heap_dump_sample_index < 0) {
    last_heap_dump_sample_index = samples_capacity_ + last_heap_dump_sample_index;
  }
...
void MemoryCache::LoadHeapDumpData(
    int64_t start_time_exl, int64_t end_time_inc, MemoryData* response) {
  std::lock_guard<std::mutex> lock(heap_dump_samples_mutex_);
