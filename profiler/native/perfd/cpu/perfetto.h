...
class Perfetto {
 public:
  ...
  virtual bool IsPerfettoRunning() { return command_.get() != nullptr && command_->IsRunning(); }

  // Stops the perfetto process. Any data gathered will remain in the output
  // file path. Stop does not kill the traced and traced_probes processes because they
  // manage ftrace and do critical book keeping for multiple trace sessions. To kill
  // the traced processes call shutdown.
  virtual void Stop() {
  ...
  // Shutdown stops the perfetto process if running as well as kills the traced and traced_probes
  // processes. Shutdown gets called when perfd dies.
 ...
 private:
  ...
  std::string GetPath(const char* executable, const std::string& abi_arch) const;
};
