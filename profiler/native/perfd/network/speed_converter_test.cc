...
TEST(SpeedConverter, NoDataAddedReturnsNoSpeed) {
  ...
  EXPECT_EQ(12345, converter.speed_time_us());
}
...
TEST(SpeedConverter, OneAddProducesExpectedSpeed) {
  ...
  converter.Add(Clock::s_to_us(1), 1024);
  EXPECT_EQ(2048, converter.speed());
  EXPECT_EQ(Clock::s_to_us(1), converter.speed_time_us());
}

TEST(SpeedConverter, TwoAddsProducesExpectedSpeed) {
  SpeedConverter converter(0, 0);
  converter.Add(Clock::s_to_us(1), 1024); // Final speek, 2K / sec
  converter.Add(Clock::s_to_us(2), 1024 + 2048); // Maintain 2K / sec

  EXPECT_EQ(2048, converter.speed());
  EXPECT_EQ(Clock::s_to_us(2), converter.speed_time_us());
}
...
TEST(SpeedConverter, SpeedCanDropToZero) {
  ...
  converter.Add(Clock::s_to_us(2), 1024 + 400);

  EXPECT_EQ(0, converter.speed());
  EXPECT_LT(Clock::s_to_us(1), converter.speed_time_us());
  EXPECT_GT(Clock::s_to_us(2), converter.speed_time_us());

  // Speed stays at 0...
  converter.Add(Clock::s_to_us(3), 1024 + 400);
  EXPECT_EQ(0, converter.speed());
  EXPECT_EQ(Clock::s_to_us(3), converter.speed_time_us());
}
