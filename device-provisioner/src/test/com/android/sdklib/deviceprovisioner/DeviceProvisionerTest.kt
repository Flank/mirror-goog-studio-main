...
class DeviceProvisionerTest {
  ...
  fun physicalUsbWiFiProperties() {
    ...
    // Use drop to skip sending the initial state; we only want changes
    fakeSession.scope.launch { provisioner.devices.drop(1).collect { channel.send(it) } }
      ...
      val originalHandle =
        channel.receive().let { handles ->
          ...
          fakeSession.scope.launch {
            handle.stateFlow.drop(1).collect { channel.send(provisioner.devices.value) }
          }
  ...
  @Test
  fun defaultDeviceDisappearsOnReconnection() {
    val channel = Channel<List<DeviceHandle>>(1)
    // Use drop to skip sending the initial state; we only want changes
    fakeSession.scope.launch { provisioner.devices.drop(1).collect { channel.send(it) } }
